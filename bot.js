...
function getBlacklist() {
  // Normalize semua entry ke number agar perbandingan konsisten
  return JSON.parse(fs.readFileSync(blacklistDB)).map(id => Number(id));
}
...
function isBlacklisted(groupId) {
  return getBlacklist().includes(Number(groupId));
}
...
// Premium Permanent helpers
function getPermPremium() {
  return JSON.parse(fs.readFileSync(permPremiumDB)).map(id => Number(id));
}
function addPermPremium(userId) {
  const id = Number(userId);
  const data = getPermPremium();
  if (!data.includes(id)) {
    data.push(id);
...
function removePermPremium(userId) {
  const id = Number(userId);
  let data = getPermPremium();
  data = data.filter(x => x !== id);
  fs.writeFileSync(permPremiumDB, JSON.stringify(data, null, 2));
}
function isPermPremium(userId) {
  return getPermPremium().includes(Number(userId));
}
...
💎 Jenis Premium : Premium ${label}`;
        ...
        const CHANNEL_ID = -1003940104613; // ganti ID channel kamu
...
bot.on("my_chat_member", async (update) => {
  ...
  if (chat.type === "group" || chat.type === "supergroup") {
    ...
    if (newStatus === "kicked" || newStatus === "left") {
      ...
      // ❌ TIDAK otomatis blacklist — blacklist hanya via /addbl manual oleh developer
...
// Pesan sambutan
// =============================
const logoUrl = "https://files.catbox.moe/455tga.jpg";
...
bot.onText(/^\/start$/, async (msg) => {
  ...
  else if (isPermPremium(userId)) roleDisplay = "Premium Permanent";
...
bot.on("callback_query", async (query) => {
  ...
  if (query.data === "check_join") {
    ...
    else if (isPermPremium(userId)) roleDisplayJoin = "Premium Permanent";
...
✰ 𝚁𝙾𝙻𝙴 : ${roleDisplayJoin}</blockquote>`,
  ...
  if (query.data === "all_menu") {
    ...
    // Premium Permanent: hanya tampilkan /share dan /share2
    if (isPermPremium(userId) && !isOwner(userId) && !isDeveloper(userId)) {
      bot.sendPhoto(chatId, logoUrl, {
        caption: `<blockquote>𝗙𝗜𝗧𝗨𝗥 𝗝𝗔𝗦𝗛𝗘𝗥 𝗠𝗘𝗡𝗨</blockquote>
<blockquote>/share -> 𝚂𝙷𝙰𝚁𝙴 𝙲𝙾𝙿𝚈 + 𝚂𝙴𝙱𝙰𝚁
/share2 -> 𝚂𝙷𝙰𝚁𝙴 𝙵𝙾𝚁𝙴𝚆𝙳 + 𝚂𝙴𝙱𝙰𝚁</blockquote>`,
        parse_mode: "HTML",
        reply_markup: { inline_keyboard: buttons }
      });
    } else {
...
/autobc -> 𝙰𝚄𝚃𝙾 𝙱𝚁𝙾𝙰𝙳𝙲𝙰𝚂𝚃 𝙻𝙾𝙾𝙿𝙸𝙽𝙶
/stopautobc -> 𝚂𝚃𝙾𝙿 𝙰𝚄𝚃𝙾 𝙱𝚁𝙾𝙰𝙳𝙲𝙰𝚂𝚃
/statusautobc -> 𝚂𝚃𝙰𝚃𝚄𝚂 𝙰𝚄𝚃𝙾 𝙱𝙲
/addch [id] -> 𝚃𝙰𝙼𝙱𝙰𝙷 𝙲𝙷𝙰𝙽𝙽𝙴𝙻 𝙱𝙾𝚃</blockquote>`,
      parse_mode: "HTML",
      reply_markup: { inline_keyboard: buttons }
    });
...
/setcdbc [durasi] -> 𝙰𝚃𝚄𝚁 𝙳𝙴𝙻𝙰𝚈 𝙻𝙾𝙾𝙿𝙸𝙽𝙶 𝙰𝚄𝚃𝙾𝙱𝙲
...
/addpremium [id] -> 𝙿𝚁𝙴𝙼𝙸𝚄𝙼 𝙿𝙴𝚁𝙼𝙰𝙽𝙴𝙽𝚃
/delpermium [id] -> 𝙷𝙰𝙿𝚄𝚂 𝙿𝙴𝚁𝙼 𝙿𝚁𝙴𝙼𝙸𝚄𝙼
/listpermium -> 𝙻𝙸𝙷𝙰𝚃 𝙿𝙴𝚁𝙼 𝙿𝚁𝙴𝙼𝙸𝚄𝙼
...
/listutang -> 𝙻𝙸𝙷𝙰𝚃 𝚄𝚃𝙰𝙽𝙶</blockquote>`,
  ...
  if (query.data === "back") {
    ...
    else if (isPermPremium(userId)) roleDisplayBack = "Premium Permanent";
...
bot.onText(/^\/share2$/, async (msg) => {
  ...
  // Premium Permanent bisa akses /share2
  // Premium manual, owner, developer juga bisa
  if (!isManualPremium(userId) && !isPermPremium(userId)) {
    return bot.sendMessage(chatId, "<blockquote>❌ Hanya Premium (manual/permanent), Owner, dan Developer yang bisa pakai.</blockquote>", { parse_mode: "HTML" });
  }
...
// =============================
// Developer command: /addpremium [id] — tambah Premium Permanent
// =============================
bot.onText(/^\/addpremium (\d+)$/, async (msg, match) => {
  if (!isDeveloper(msg.from.id)) return;
  const targetId = parseInt(match[1]);

  addPermPremium(targetId);

  // Ambil username bot untuk button
  const botInfo = await bot.getMe();
  const botUsername = botInfo.username;

  const notifText =
    `<blockquote>${targetId} telah ditambahkan kedalam premium ✅
` +
    `Batas waktu : 9999Day
` +
...
` +
    `Anda dapat menggunakan fitur premium sekarang! start bot di bawah ini untuk menggunakannya 👇</blockquote>`;

  bot.sendMessage(msg.chat.id, notifText, {
    parse_mode: "HTML",
    reply_markup: {
      inline_keyboard: [[
        { text: "START BOT", url: `https://t.me/${botUsername}?start=start` }
      ]]
    }
  });
});

// Developer command: /delpermium [id] — hapus Premium Permanent
bot.onText(/^\/delpermium (\d+)$/, (msg, match) => {
  if (!isDeveloper(msg.from.id)) return;
  const targetId = parseInt(match[1]);

  if (!isPermPremium(targetId)) {
    return bot.sendMessage(msg.chat.id, `<blockquote>❌ User <code>${targetId}</code> tidak ada di Premium Permanent.</blockquote>`, { parse_mode: "HTML" });
  }

  removePermPremium(targetId);
  bot.sendMessage(msg.chat.id, `<blockquote>🗑 User <code>${targetId}</code> dihapus dari Premium Permanent.</blockquote>`, { parse_mode: "HTML" });
});

// Developer command: /listpermium — list semua Premium Permanent
bot.onText(/^\/listpermium$/, (msg) => {
  if (!isDeveloper(msg.from.id)) return;
  const data = getPermPremium();
  if (data.length === 0) {
    return bot.sendMessage(msg.chat.id, "<blockquote>📭 Tidak ada user Premium Permanent.</blockquote>", { parse_mode: "HTML" });
  }
  let listText = "<blockquote>PREMIUM PERMANENT LIST</blockquote>\n\n";
  data.forEach((id, i) => {
    listText += `<blockquote>${i + 1}. <code>${id}</code> — 9999Day</blockquote>\n`;
  });
  listText += `\n<blockquote>Total: ${data.length} user</blockquote>`;
  bot.sendMessage(msg.chat.id, listText.trim(), { parse_mode: "HTML" });
});
...
// Cooldown map untuk /bcuser
const bcuserFreeCooldown = new Map();    // premium gratisan: 1 jam
const bcuserManualCooldown = new Map();  // premium manual: 20 menit
const BCUSER_FREE_COOLDOWN_MS   = 60 * 60 * 1000; // 1 jam
const BCUSER_MANUAL_COOLDOWN_MS = 20 * 60 * 1000; // 20 menit

function formatSisa(ms) {
  const totalMenit = Math.ceil(ms / 60000);
  const jam = Math.floor(totalMenit / 60);
  const menit = totalMenit % 60;
  return jam > 0 ? `${jam} jam ${menit} menit` : `${totalMenit} menit`;
}
...
bot.onText(/^\/bcuser$/, async (msg) => {
  ...
  // Cek cooldown 1 jam untuk premium gratisan
  if (isFreePremium(senderId)) {
    const lastUsed = bcuserFreeCooldown.get(senderId);
    if (lastUsed && now - lastUsed < BCUSER_FREE_COOLDOWN_MS) {
      const sisaText = formatSisa(BCUSER_FREE_COOLDOWN_MS - (now - lastUsed));
      return bot.sendMessage(msg.chat.id, `<blockquote>⏳ Kamu bisa menggunakan /bcuser lagi dalam <b>${sisaText}</b>.\n\nPremium gratisan hanya bisa broadcast 1x per jam.</blockquote>`, { parse_mode: "HTML" });
    }
    bcuserFreeCooldown.set(senderId, now);
  }

  // Cek cooldown 20 menit untuk premium manual (bukan owner/developer)
  if (isManualPremium(senderId) && !isOwner(senderId)) {
    const lastUsed = bcuserManualCooldown.get(senderId);
    if (lastUsed && now - lastUsed < BCUSER_MANUAL_COOLDOWN_MS) {
      const sisaText = formatSisa(BCUSER_MANUAL_COOLDOWN_MS - (now - lastUsed));
      return bot.sendMessage(msg.chat.id, `<blockquote>⏳ Kamu bisa menggunakan /bcuser lagi dalam <b>${sisaText}</b>.\n\nPremium hanya bisa broadcast 1x per 20 menit.</blockquote>`, { parse_mode: "HTML" });
    }
    bcuserManualCooldown.set(senderId, now);
  }
...
bot.onText(/^\/pay$/, (msg) => {
  ...
  const buttons = [[{ text: "👤 OWNER", url: "https://t.me/axcelforlife" }]];

  // Kirim foto QRIS terlebih dahulu (bisa URL atau path lokal)
  const qrisPhoto = "https://files.catbox.moe/vl13aq.jpg"; // ganti dengan URL/file QRIS kamu
...
bot.onText(/^\/copyweb (.+)$/, async (msg, match) => {
  ...
  // cek join channel
  try {
    const member = await bot.getChatMember("@limakoshongenam", userId);
    if (["left", "kicked"].includes(member.status)) {
      const buttons = [[{ text: "📢 JOIN CHANNEL", url: "https://t.me/limakoshongenam" }]];
      return bot.sendMessage(
        chatId,
        "⚠️ Untuk menggunakan fitur ini kamu harus join channel 📢 @limakoshongenam",
        { reply_markup: { inline_keyboard: buttons } }
      );
    }
  } catch (e) {
...
// /deladdbl -> developer/owner, bisa di grup ATAU private dengan argumen ID
bot.onText(/^\/deladdbl(?:\s+(-?\d+))?$/, async (msg, match) => {
  if (!isDeveloper(msg.from.id) && !isOwner(msg.from.id)) return;

  const isPrivate = msg.chat.type === "private";
  const argId = match[1] ? Number(match[1]) : null;

  // Tentukan target grup yang akan dihapus dari blacklist
  let targetId;
  if (isPrivate) {
    // Di private: wajib ada argumen ID
    if (!argId) {
      return bot.sendMessage(msg.chat.id,
        "<blockquote>⚠️ Gunakan format: <code>/deladdbl [id grup]</code>\nContoh: <code>/deladdbl -1001234567890</code></blockquote>",
        { parse_mode: "HTML" });
    }
    targetId = argId;
  } else {
    // Di grup: pakai chat ID grup itu sendiri
    if (msg.chat.type !== "group" && msg.chat.type !== "supergroup") {
      return bot.sendMessage(msg.chat.id, "<blockquote>⚠️ Perintah ini hanya bisa digunakan di grup atau private dengan argumen ID.</blockquote>", { parse_mode: "HTML" });
    }
    targetId = msg.chat.id;
  }

  if (!isBlacklisted(targetId)) {
    return bot.sendMessage(msg.chat.id, `<blockquote>❌ Grup <code>${targetId}</code> tidak ada di blacklist.</blockquote>`, { parse_mode: "HTML" });
  }
  ...
  bot.sendMessage(msg.chat.id,
    `<blockquote>🗑 Grup "${esc(groupName)}" (<code>${targetId}</code>) berhasil dihapus dari blacklist.</blockquote>`,
    { parse_mode: "HTML" });

  // Notif admin utama (kalau bukan admin sendiri yang jalankan)
  if (msg.from.id !== ADMIN_ID) {
    bot.sendMessage(ADMIN_ID,
      `<blockquote>✅ Grup dihapus dari blacklist</blockquote>\n<blockquote>👥 ${esc(groupName)}</blockquote>\n<blockquote>🆔 <code>${targetId}</code></blockquote>`,
      { parse_mode: "HTML" });
  }
});
...
// =============================
// AUTO BC SYSTEM (/autobc) — Owner & Developer only
// Broadcast ke semua user (looping seperti autoshare)
// =============================
let autoBcInterval = null;
let autoBcCooldown = 10 * 60 * 1000; // default 10 menit
let autoBcMessage = null;
let autoBcFromChatId = null;
let autoBcRunning = false;

// Fungsi broadcast ke semua user
async function broadcastToAllUsers(message, fromChatId) {
  const data = getUsers();
  let success = 0, failed = 0;
  for (const userId of data) {
    try {
      await bot.copyMessage(userId, fromChatId, message.message_id);
      success++;
    } catch { failed++; }
    await new Promise(r => setTimeout(r, 200));
  }
...
// /autobc — mulai auto broadcast ke user (owner & developer only)
bot.onText(/^\/autobc$/, async (msg) => {
  const userId = msg.from.id;
  if (!isOwner(userId) && !isDeveloper(userId)) {
    return bot.sendMessage(msg.chat.id, "<blockquote>❌ Hanya Owner dan Developer yang bisa menggunakan /autobc.</blockquote>", { parse_mode: "HTML" });
  }
  if (!msg.reply_to_message) {
    return bot.sendMessage(msg.chat.id, "<blockquote>⚠️ Harus reply pesan untuk /autobc</blockquote>", { parse_mode: "HTML" });
  }
  if (autoBcInterval) {
    return bot.sendMessage(msg.chat.id, "<blockquote>⚠️ AutoBC sudah aktif.\nGunakan /stopautobc untuk menghentikan.</blockquote>", { parse_mode: "HTML" });
  }

  autoBcMessage = msg.reply_to_message;
  autoBcFromChatId = msg.chat.id;

  const cycle = async () => {
    if (autoBcRunning) return;
    autoBcRunning = true;
    const result = await broadcastToAllUsers(autoBcMessage, autoBcFromChatId);
    bot.sendMessage(msg.chat.id,
      `<blockquote>📢 AutoBC — putaran selesai</blockquote>\n<blockquote>✅ Berhasil: ${result.success}</blockquote>\n<blockquote>❌ Gagal: ${result.failed}</blockquote>\n<blockquote>👤 Total: ${result.total}</blockquote>`,
      { parse_mode: "HTML" }
    ).catch(() => {});
    autoBcRunning = false;
  };
  ...
  // looping
  autoBcInterval = setInterval(cycle, autoBcCooldown);

  bot.sendMessage(msg.chat.id,
    `<blockquote>✅ AutoBC dimulai</blockquote>\n<blockquote>⏳ Cooldown antar putaran: ${formatDuration(autoBcCooldown)}</blockquote>`,
    { parse_mode: "HTML" }
  );
});

// /stopautobc — hentikan auto broadcast
bot.onText(/^\/stopautobc$/, (msg) => {
  const userId = msg.from.id;
  if (!isOwner(userId) && !isDeveloper(userId)) {
    return bot.sendMessage(msg.chat.id, "<blockquote>❌ Hanya Owner dan Developer yang bisa menggunakan /stopautobc.</blockquote>", { parse_mode: "HTML" });
  }
  if (!autoBcInterval) {
    return bot.sendMessage(msg.chat.id, "<blockquote>⚠️ AutoBC belum aktif.</blockquote>", { parse_mode: "HTML" });
  }

  clearInterval(autoBcInterval);
  autoBcInterval = null;
  autoBcMessage = null;
  autoBcFromChatId = null;
  autoBcRunning = false;

  bot.sendMessage(msg.chat.id, "<blockquote>🛑 AutoBC dihentikan.</blockquote>", { parse_mode: "HTML" });
});

// /setcdbc <durasi> — atur delay looping autobc (developer only)
bot.onText(/^\/setcdbc (.+)$/, (msg, match) => {
  if (!isDeveloper(msg.from.id)) return;
  const input = match[1].trim();
  const ms = parseDuration(input);
  if (!ms) {
    return bot.sendMessage(msg.chat.id, "<blockquote>⚠️ Format salah.\nContoh: 30s, 5m, 2h, 1h30m</blockquote>", { parse_mode: "HTML" });
  }
  autoBcCooldown = ms;

  // update interval jika autobc sedang aktif
  if (autoBcInterval) {
    clearInterval(autoBcInterval);
    autoBcInterval = setInterval(async () => {
      if (autoBcRunning) return;
      autoBcRunning = true;
      const result = await broadcastToAllUsers(autoBcMessage, autoBcFromChatId);
      bot.sendMessage(autoBcFromChatId,
        `<blockquote>📢 AutoBC — putaran selesai</blockquote>\n<blockquote>✅ Berhasil: ${result.success}</blockquote>\n<blockquote>❌ Gagal: ${result.failed}</blockquote>\n<blockquote>👤 Total: ${result.total}</blockquote>`,
        { parse_mode: "HTML" }
      ).catch(() => {});
      autoBcRunning = false;
  ...
  bot.sendMessage(msg.chat.id,
    `<blockquote>✅ Delay AutoBC diatur ke ${formatDuration(ms)}</blockquote>`,
    { parse_mode: "HTML" }
  );
});

// /statusautobc — cek status autobc
bot.onText(/^\/statusautobc$/, (msg) => {
  const userId = msg.from.id;
  if (!isOwner(userId) && !isDeveloper(userId)) return;

  const status = autoBcInterval ? "🟢 Aktif" : "🔴 Tidak aktif";
  const totalUser = getUsers().length;
  bot.sendMessage(msg.chat.id,
    `<blockquote>📡 STATUS AUTO BC</blockquote>\n<blockquote>Status : ${status}</blockquote>\n<blockquote>⏳ Cooldown : ${formatDuration(autoBcCooldown)}</blockquote>\n<blockquote>👤 Total user : ${totalUser}</blockquote>`,
    { parse_mode: "HTML" }
  );
});
